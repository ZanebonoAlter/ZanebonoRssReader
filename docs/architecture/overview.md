...
OpenTelemetry 集成（`backend-go/internal/platform/tracing/`），HTTP 请求和调度器入口自动创建 span，自定义 SQLite Span Exporter 落库，提供 trace 查询和统计 API。
...
│       ├── domain/           # 业务域（13 个子包：feeds, articles, categories, summaries, contentprocessing, digest, preferences, aiadmin, models, topictypes, topicextraction, topicanalysis, topicgraph）
│       ├── jobs/             # 调度外壳（6 类定时任务 + handler）
...
- **统一调度器管理**：6 类后台任务通过统一 runtime 启动，统一 `/api/schedulers/*` API 查询状态和手动触发

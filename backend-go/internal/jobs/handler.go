...
func GetSchedulersStatus(c *gin.Context) {
	...
	if status := safeGetStatus(runtimeinfo.AutoRefreshSchedulerInterface, "auto_refresh", "Auto-refresh RSS feeds"); status != nil {
		schedulers = append(schedulers, status)
	}

	if status := safeGetStatus(runtimeinfo.AutoSummarySchedulerInterface, "auto_summary", "Auto-generate AI summaries for feeds"); status != nil {
		schedulers = append(schedulers, status)
	}

	if status := safeGetStatus(runtimeinfo.AISummarySchedulerInterface, "ai_summary", "AI summarize Firecrawl content"); status != nil {
		schedulers = append(schedulers, status)
	}
...
func GetSchedulerStatus(c *gin.Context) {
	...
	switch name {
	case "auto_refresh":
		scheduler = runtimeinfo.AutoRefreshSchedulerInterface
		description = "Auto-refresh RSS feeds"
	case "auto_summary":
		scheduler = runtimeinfo.AutoSummarySchedulerInterface
		description = "Auto-generate AI summaries for feeds"
	case "ai_summary":
		scheduler = runtimeinfo.AISummarySchedulerInterface
		description = "AI summarize Firecrawl content"
	case "firecrawl":
		scheduler = runtimeinfo.FirecrawlSchedulerInterface
...
func TriggerScheduler(c *gin.Context) {
	...
	switch name {
	case "auto_refresh":
		if runtimeinfo.AutoRefreshSchedulerInterface != nil {
			if scheduler, ok := runtimeinfo.AutoRefreshSchedulerInterface.(interface{ TriggerNow() map[string]interface{} }); ok {
				respondTriggerResult(c, name, scheduler.TriggerNow())
				return
			}
			if scheduler, ok := runtimeinfo.AutoRefreshSchedulerInterface.(interface{ Trigger() }); ok {
	...
	case "auto_summary":
		if runtimeinfo.AutoSummarySchedulerInterface != nil {
			if scheduler, ok := runtimeinfo.AutoSummarySchedulerInterface.(interface{ TriggerNow() map[string]interface{} }); ok {
				respondTriggerResult(c, name, scheduler.TriggerNow())
				return
			}
		}
	case "ai_summary":
		if runtimeinfo.AISummarySchedulerInterface != nil {
			if scheduler, ok := runtimeinfo.AISummarySchedulerInterface.(interface{ Trigger() }); ok {
	...
	case "firecrawl":
		if runtimeinfo.FirecrawlSchedulerInterface != nil {
			if scheduler, ok := runtimeinfo.FirecrawlSchedulerInterface.(interface{ Trigger() }); ok {

...
type ContentCompletionScheduler struct {
	...
	isRunning         bool
	isExecuting       bool
	currentArticle    *services.ContentCompletionArticleRef
	lastProcessed     *services.ContentCompletionArticleRef
	lastError         string
...
type ContentCompletionRunSummary struct {
	...
	CurrentArticle         *services.ContentCompletionArticleRef `json:"current_article,omitempty"`
	LastProcessed          *services.ContentCompletionArticleRef `json:"last_processed,omitempty"`
	StaleProcessingArticle *services.ContentCompletionArticleRef `json:"stale_processing_article,omitempty"`
...
func (s *ContentCompletionScheduler) checkAndCompleteArticles() {
	...
	for _, article := range articles {
		s.mu.Lock()
		s.currentArticle = services.ToArticleRef(article)
		...
		if err := s.completionService.CompleteArticle(article.ID); err != nil {
			...
			s.mu.Lock()
			s.lastError = err.Error()
			s.lastProcessed = services.ToArticleRef(article)

...
func defaultDigestConfig() digest.DigestConfig {
	return digest.DigestConfig{
		DailyEnabled:         false,
		DailyTime:            "09:00",
		WeeklyEnabled:        false,
		WeeklyDay:            1,
...
func ensureDigestConfig() (*digest.DigestConfig, error) {
...
func totalSummaryCount(digests []digest.CategoryDigest) int {
	total := 0
...
func buildDigestMarkdown(kind string, date time.Time, digests []digest.CategoryDigest) string {
...
func buildPreviewCategories(digests []digest.CategoryDigest) ([]digestPreviewCategory, *uint, *uint) {
...
func buildPreview(kind string, date time.Time) (*digestPreviewResponse, *digest.DigestConfig, []digest.CategoryDigest, error) {
	config, err := ensureDigestConfig()
	if err != nil {
		return nil, nil, nil, err
	}

	generator := digest.NewDigestGenerator(config)
	var digests []digest.CategoryDigest
...
func GetDigestStatus(c *gin.Context) {
	if DigestSchedulerInterface != nil {
		if scheduler, ok := DigestSchedulerInterface.(interface{ GetStatus() map[string]interface{} }); ok {
			c.JSON(http.StatusOK, gin.H{
				"success": true,
...
func UpdateDigestConfig(c *gin.Context) {
	...
	if DigestSchedulerInterface != nil {
		if scheduler, ok := DigestSchedulerInterface.(interface{ Reload() error }); ok {
...
func TestFeishuPush(c *gin.Context) {
	...
	notifier := digest.NewFeishuNotifier(webhookURL)
...
func TestObsidianWrite(c *gin.Context) {
	...
	exporter := digest.NewObsidianExporter(vaultPath)

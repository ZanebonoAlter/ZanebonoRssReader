...
| 8 | 标签树增强与图谱交互优化 | 标签简介提取、时间筛选、抽象标签图谱展示、合并预览迁移、节点手动归类 | TBD | TBD |
...
7. 标签简介提取有效，标签树时间筛选可用，抽象标签图谱可视化正确，合并预览迁移至设置页完成，节点手动归类可用

### Phase 8: 标签树增强与图谱交互优化

**Goal**: 完善标签树与图谱的交互体验：标签简介提取补充 LLM 上下文、时间维度筛选、抽象标签在图谱中可视化、合并预览功能迁移至设置页、节点手动归类

**Depends on**: Phase 7 (抽象标签层级关系 + 图谱基础设施)

**Success Criteria:**
1. 提取标签和抽象标签时均提取简介(description)，用于 LLM 上下文补充——文章标签简介用于抽象层聚合，抽象标签简介用于二次抽象
2. 标签树界面支持时间维度筛选，避免历史标签干扰判断
3. 抽象维度标签在主题图谱节点上用特殊颜色展示，点击后时间线支持子标签筛选
4. 图谱界面的"标签合并预览"功能移至 GlobalSettings 的 merge 队列旁（调研其作用范围，评估是否触发抽象层重建）
